...
def gate_sidecars() -> dict[str, dict[str, Any]]:
    """The published verdicts. `artifacts/` is gitignored, so this is an overlay.

    A clean checkout has no artifacts/ and the page must still build - it says
    `unknown` for the gate rows rather than failing, and says WHY on the page so
    nobody reads an absence as a pass.
    """
    ...
    for role in ("validator", "identifier"):
        for candidate in (ARTIFACTS / "gate" / f"{role}-v1.json", ARTIFACTS / "local" / f"{role}-v1.json"):
            d = _load(candidate)
            if d is not None:
                found[role] = {**d, "_source": str(candidate.relative_to(REPO_ROOT)).replace("\\", "/")}
                break
    return found
...
def render(data: dict[str, Any]) -> str:
    ...
    for role in ("validator", "identifier"):
        ...
        if not s:
            gate_rows.append(
                ...
                f"<span class='note'>no sidecar on disk. <code>artifacts/</code> is gitignored, "
                f"so a clean checkout has none - this is an absence, not a pass.</span></td></tr>"
            )
            continue
    ...
    return f"""<style>
  :root {{
    --bg:#fbfaf7; --fg:#16181c; --muted:#5d6470; --line:#e2ded4; --card:#fff;
    --pass:#1e7a3c; --fail:#c1272d; --warn:#a86a10; --accent:#5b3df5;
  }}
  @media (prefers-color-scheme: dark) {{
    :root:not([data-theme="light"]) {{
      --bg:#14161a; --fg:#f2f0ea; --muted:#9aa2b1; --line:#2b2f37; --card:#1b1e24;
      --pass:#5ad48b; --fail:#ff8a86; --warn:#e8b45c; --accent:#a99bff;
    }}
  }}
  ...
  h2 {{ font-size:1.12rem; margin:38px 0 10px; letter-spacing:-0.01em; }}
  .sub {{ color:var(--muted); margin:0 0 26px; }}
  section {{ background:var(--card); border:1px solid var(--line); border-radius:12px;
    padding:16px 18px; margin-bottom:18px; }}
  table {{ width:100%; border-collapse:collapse; font-size:13.5px; }}
  th,td {{ text-align:start; padding:7px 9px; border-block-end:1px solid var(--line);
    vertical-align:top; }}
  ...
  .prov {{ font-size:11px; padding:1px 6px; border-radius:999px; border:1px solid var(--line); }}
  .prov.rep {{ color:var(--pass); border-color:currentColor; }}
  .prov.proxy {{ color:var(--warn); border-color:currentColor; }}
  .bar {{ display:inline-block; inline-size:110px; block-size:7px; background:var(--line);
  ...
  .lead {{ border-inline-start:3px solid var(--accent); padding-inline-start:12px; }}
</style>
<div class="wrap">
<h1>Smart Bin Recognition — evidence</h1>
<p class="sub">Generated from the repository at <b>{e(generated)}</b> by
<code>ml/scripts/build_dashboard.py</code>. Nothing here is typed by hand.
A figure whose source file is missing renders as <span class="unknown">unknown</span>.</p>
