...
repository as of <b>{e(generated)}</b> - the date of the commit, not of the
render, so this page rebuilds byte-for-byte from a clean checkout. Nothing here
is typed by hand, and a figure whose source file is missing renders as
...
def _commit_stamp() -> str:
    """When the SOURCES were last changed, not when this ran.

    `datetime.now()` would make the output differ on every run, which defeats the
    CI check that keeps the committed page honest against the files it claims to
    read. The commit date is deterministic for a given tree and is the more
    useful fact anyway: it dates the evidence rather than the render.
    """
    import subprocess
